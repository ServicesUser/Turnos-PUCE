...
class TurneroController extends Controller
{
    ...
    public function tiempoPuesto(Request $datos){
        ...
        if ($validator->fails()) {
            ...
            $cubiculo=$datos->modulo ? "and cubiculos.id_cu=".$datos->modulo : '';
            $toca   =   DB::select("SELECT turnos.id_tu,nu_tu,turnos.id_et,inicio_tu,fin_tu,estudiantes.cedula_es,nombres_es,apellidos_es,email_es,detalle_cu,fecha_tu,nombre_et FROM turnos natural join horarios natural join users natural join estudiantes natural join  cubiculos natural join estado_turno where DATE_FORMAT(CONCAT(fecha_tu,' ',inicio_tu), '%Y-%m-%d %H:%i:%s')>=DATE_FORMAT('$fecha','%Y-%m-%d %H:%i:%s') and id_et=2 and fecha_tu=DATE('$fecha') $cubiculo order by inicio_tu,detalle_cu ASC ");
            $paso   =   DB::select("SELECT turnos.id_tu,nu_tu,turnos.id_et,inicio_tu,fin_tu,estudiantes.cedula_es,nombres_es,apellidos_es,email_es,detalle_cu,fecha_tu,nombre_et FROM turnos natural join horarios natural join users natural join  cubiculos natural join estado_turno left join estudiantes on estudiantes.cedula_es=turnos.cedula_es where DATE_FORMAT(CONCAT(fecha_tu,' ',fin_tu), '%Y-%m-%d %H:%i:%s')<=DATE_FORMAT('$fecha2','%Y-%m-%d %H:%i:%s') and id_et!=2 and fecha_tu=DATE('$fecha') $cubiculo order by inicio_tu,detalle_cu ASC");
