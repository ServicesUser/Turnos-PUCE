...
class TurneroController extends Controller
{
    ...
    public function tiempoPuesto(Request $datos){
        ...
        if ($validator->fails()) {
            ...
            $fecha  =   Date::createFromFormat('Y-m-d H:i:s',$datos-> fechaInicio." ".$datos-> desde.":00:00");
            
            $fecha2 =   Date::createFromFormat('Y-m-d H:i:s',$datos-> fechaFin." ".$datos-> hasta.":00:00");
            ...
            $toca   =   DB::select(
                        "SELECT turnos.id_tu,
                                ...
                                inicio_tu,
                                fin_tu,
                                estudiantes.cedula_es,
                                nombres_es,apellidos_es,
                                email_es,
                                detalle_cu,
                                fecha_tu,
                                nombre_et FROM turnos 
                                                ...
                                                and DATE_FORMAT(CONCAT(fecha_tu,' ',inicio_tu), '%Y-%m-%d %H:%i:%s') <=DATE_FORMAT('$fecha2','%Y-%m-%d %H:%i:%s')


                                                 and 
                                                id_et=2 

                                                /* and fecha_tu = DATE('$fecha') $cubiculo */
            ...
            $paso=  DB::select(
                    "SELECT turnos.id_tu,
                            ...
                            where 
                                ...
                                and
                                DATE_FORMAT(CONCAT(fecha_tu,' ',fin_tu), '%Y-%m-%d %H:%i:%s') <= DATE_FORMAT('$fecha2','%Y-%m-%d %H:%i:%s') 
                                and
                                id_et!=2 
                                /*and 
                                fecha_tu = DATE('$fecha') $cubiculo */
                                order by inicio_tu,detalle_cu ASC");
